...
export default (props) => {
  ...
  const handleSurveyClick = () =>
    surveyMonkeyRef.current.showSurveyMonkey('7GYZD8K');
  ...
  const init = () => {
    ...
    getTips()
      .then((tsData) => {
        ...
        let cols = {};
        tsData.categories.forEach((c) => {
          cols[c.name] = c.colour;
        });
        setColours(cols);
        setCategories(cats);
    ...
    <SafeAreaView style={styles.view}>
      <View style={styles.about}>
        ...
        <ScrollView
          ...
          {tips && <Carousel key={key} slideList={tips} colours={colours} />}
          <View style={styles.tips_container}>
            <View style={styles.categories}>
              <View style={styles.filter_text}>
                <Text style={[styles.filter_text_text, {fontWeight: 'bold'}]}>
                  Filter By
                </Text>
                <TouchableOpacity
              ...
              {categories.map((category, index) => {
                ...
                return (
                  <TouchableOpacity
                    style={[
                      styles.category_filter,
                      {borderColor: catColour(category)},
                      selected && {backgroundColor: catColour(category)},
                    ]}
                    key={index}
                    onPress={() => filterCats(category)}>
                    <Text
                      style={[
                        styles.category_filter_text,
                        selected && styles.category_filter_text_selected,
                      ]}>
                      {category}
                    </Text>
                  </TouchableOpacity>
            ...
            <Head style={{textAlign: 'center'}}>
              Got a tip to share with the community?
            </Head>
            <Para style={{textAlign: 'center'}}>
              Let us know how you become more waste responsible by using the
              form below!
            </Para>
            <View style={styles.submit_links}>
              <TouchableOpacity
                onPress={handleSurveyClick}
                style={styles.submit_link_button}>
