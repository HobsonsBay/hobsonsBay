...
import { getItemBinLabel, getBinImgNT, getItemBinColor, getItemTextColor, RUBBISH } from './utils';
...
export default (props) => {
  ...
  const itemCardStyle = [
    styles.item_card,
    { backgroundColor: getItemBinColor(item.bin_type), color: getItemTextColor(item.bin_type) }
  ];
  ...
  return (
    <SafeAreaView style={styles.view}>
      <View style={styles.item}>
        ...
        <ScrollView style={styles.item_body} contentContainerStyle={[styles.item_body_content, loading ? { flex: 1 } : {}]}>
          {loading && <ActivityIndicator size='large' color='#f0b41c' />}
          {!loading &&
            <>
              <View style={styles.item_bin}>
                <Image style={styles.item_bin_img} source={images[getBinImgNT(item.bin_type)]} />
                <Text style={styles.item_bin_label}>{getItemBinLabel(item.bin_type)}</Text>
              </View>
              {!isEmpty(item.url) &&
                <View style={styles.item_photo}>
                  <Image style={styles.item_photo_img} source={{ uri: item.url }} resizeMode='contain' />
                </View>}
              {!isEmpty(item.description) &&
                <Text style={styles.item_description}>{trim(item.description)}</Text>}
              {!isEmpty(item.disposal_guidance) &&
                <View style={styles.item_section}>
                  <Text style={styles.item_section_head}>Disposal Guidance</Text>
                  <Paragraph text={item.disposal_guidance} />
                </View>}
              {!isEmpty(item.alternative_disposal) &&
                <View style={styles.item_section}>
                  <Text style={styles.item_section_head}>Alternative Disposal</Text>
                  <Paragraph text={item.alternative_disposal} />
                </View>}
              {!isEmpty(item.additional_info) &&
                <View style={styles.item_section}>
                  <Text style={styles.item_section_head}>Additional Info</Text>
                  <Paragraph text={item.additional_info} />
                </View>}
            </>}
        </ScrollView>
      </View>
